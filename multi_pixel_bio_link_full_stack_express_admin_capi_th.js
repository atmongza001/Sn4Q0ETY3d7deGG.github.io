// server.js — Bio/Pixel Admin (Multi-tenant) — Version 3.3 (Error Fix & Final)
...
app.get("/admin", requireAdmin, (req, res) => { const db = readDB(); const list = Object.keys(db.tenants); res.type("html").send(`<!doctype html><meta charset=utf-8><title>Admin</title><style>body{margin:0;padding:24px;background:#0b1220;color:#fff;font-family:system-ui}.row{display:flex;gap:8px;max-width:600px;margin:16px 0} .btn{padding:8px 12px;border-radius:10px;border:none;background:#374151;color:#fff;cursor:pointer; text-decoration: none;}input{padding:10px;border-radius:10px;border:1px solid #223047;background:#0f172a;color:#fff;flex:1}.tile{background:#111827;border:1px solid #223047;border-radius:12px;padding:12px;margin:6px 0;display:flex;justify-content:space-between;align-items:center}a{color:#a78bfa;text-decoration:none}</style><h1>Tenants</h1>${list.map(t => `<div class=tile><div><a href="/${t}" target="_blank">/${t}</a></div><div class="row" style="margin:0"><a href="/admin/${t}" class="btn">Edit</a></div></div>`).join("")}<form class=row method=post action=/admin/create><input name=slug placeholder="new-brand-slug" required><button class=btn>Create New</button></form><p style="margin-top:16px"><a href="/admin/logout">Logout</a></p>`); });
...
<style>
  ...
  textarea{min-height:120px; white-space: pre; overflow-wrap: normal; overflow-x: scroll;}
...
<div class=grid>
  <form class=card method=post action="/admin/${req.params.tenant}/profile">
    <h3>Profile & Theme</h3>
    <div class=row><div style="flex:1"><div class=label>Name</div><input name=name value="${esc(cfg.profile.name)}"></div><div style="flex:1"><div class=label>Subtitle</div><input name=subtitle value="${esc(cfg.profile.subtitle)}"></div></div>
  ...
  <div class=card>
    ...
    <form method=post action="/admin/${req.params.tenant}/content/add"><input type=hidden name=type value=code><textarea name=code placeholder="Custom HTML/CSS/JS Code"></textarea><button class="btn secondary" style="margin-top:8px">Add Code</button></form>
  </div>
</div>`);
});
...
function renderHTML(tenant, cfg) {
  // ⭐ START: เพิ่มฟังก์ชัน esc ที่ขาดไปกลับเข้ามา
  const esc = (s) => (s || "").toString().replace(/</g, "&lt;").replace(/>/g, "&gt;");
  // ⭐ END: สิ้นสุดการแก้ไข

  const title = cfg.seo.title || cfg.profile.name || tenant;
  const desc = cfg.seo.description || cfg.profile.subtitle || "";
  const fontLink = cfg.theme.fontURL ? `<link href="${cfg.theme.fontURL}" rel="stylesheet">` : "";
  ...
  const bodyBgColor = '#0f172a';

  const isVideo = (url = "") => /\.(mp4|webm|mov)$/i.test(url);

  function coverHTML() {
    const { avatar, cover, name, subtitle, coverStyle } = cfg.profile;
    const avatarHTML = avatar ? `<img src="${esc(avatar)}" alt="Profile Avatar" class="avatar">` : "";
    const nameHTML = name ? `<h1 class="name">${esc(name)}</h1>` : "";
    const subtitleHTML = subtitle ? `<p class="subtitle">${esc(subtitle)}</p>` : "";
    let coverMediaHTML = "";
    if (cover) {
      if (isVideo(cover)) {
        coverMediaHTML = `<video src="${esc(cover)}" class="cover-media" autoplay loop muted playsinline></video>`;
      } else {
        coverMediaHTML = `<img src="${esc(cover)}" class="cover-media" alt="Cover Image">`;
      }
    }
  ...
  const contentHTML = (cfg.content || []).map(block => {
      ...
      if (block.type === 'code') { return `<div class="content-block code-block">${block.data.code}</div>`; }
      return '';
  }).join('\n');
...
${fontLink}
<style>
  :root { --w: min(100%, 720px); --text-shadow: 0 2px 8px rgba(0, 0, 0, 0.5); --body-bg: ${bodyBgColor}; }
  * { box-sizing: border-box; }
  body { margin: 0; background: var(--body-bg); color: #e5e7eb; font-family: ${fontFamily}; }
  .wrapper { background: ${bgCSS}; }
...
app.get("/:tenant", requireAdmin, (req, res) => { ensureTenant(req.params.tenant); const cfg = getTenant(req.params.tenant); res.type("html").send(renderHTML(req.params.tenant, cfg)); });
